// import { useEffect ,useState} from "react";

import { Link } from "react-router-dom"
import "./navbar.css"
import {useNavigate} from "react-router-dom"
export default function Navbar() {
    
    //  console.log(token)
    const navigate = useNavigate();
   
    
   
   
  return (
   <div className="nav">
       <div className="topleft">

       Blog
       </div>
       <div className="topcenter">
           <ul className="navlist">
               <li className="navitems "><Link className="link"to={"/"} style={{textDecoration:"none",color:"inherit"}}>HOME</Link></li>
              
                   <li className="navitems "> <Link className="link"to={"/post"} style={{textDecoration:"none" ,color:"inherit"}}>WRITE</Link></li>
               
            
               <li className="navitems " onClick={() => {
               localStorage.clear();
               navigate("/login")
               }
            }>{ "LOGOUT"}</li>
           </ul>
       </div>
       <div className="topright">
           <img className="navimage" src="data:image/png;base64," alt=""></img>
             
             <ul className="navlist">
              <li className="navitems"> <Link className="link" to="/register">Register</Link></li>
              <li className="navitems"><Link className="link" to="/login">Login</Link></li>
              {/* <li className="navitems"><Link className="link" to="/login">Logout</Link></li> */}
             
             </ul> 
           
           
         
       </div>

   </div>
  )
}
